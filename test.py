...
for idx, (img_a, att_a, img_name) in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        samples = [img_a]
        z = F.interpolate(img_a, scale_factor=0.125, mode='bilinear')
        y = F.interpolate(z, scale_factor=8, mode='bilinear')
        '''Visi(y, '1l.jpg')
        Visi(img_a - y, '1h.jpg')
        Visi(img_a, '1x.jpg')'''
        for i, att_b in enumerate(att_b_list):
            if i == 0:
                ...
                x, _,_,_ = attgan.G(img_a, att_b_, att_b_)
                #Visi(img_a - x, 'input.jpg')
                samples.append(x)
                #samples.append(img_a - x)
                #samples.append(img_a - x)
                #samples.append(img_a - y)
            else:
                att_b_ = (att_b * 2 - 1) * args.thres_int  # [-0.5,0.5]
                ...
                # t=att_b_ - a_
                # t[t==-1]=0
                x, _,_,_= attgan.G(img_a, att_b_, a_)
                samples.append(x)
