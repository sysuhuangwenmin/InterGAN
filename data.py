...
class CelebA_t(data.Dataset):
    def __init__(self, data_path, attr_path, image_size, mode, selected_attrs):
        super(CelebA_t, self).__init__()
        self.data_path = data_path
        att_list = open(attr_path, 'r', encoding='utf-8').readlines()[1].split()
        atts = [att_list.index(att) + 1 for att in selected_attrs]
        images = np.loadtxt(attr_path, skiprows=2, usecols=[0], dtype=np.str)
...
class FFHQ(data.Dataset):
    def __init__(self, data_path, attr_path,image_size, mode, selected_attrs):
        ...
        if mode == 'train':
            self.images = orig_images[:65332]
            self.labels = orig_labels[:65332]
        if mode == 'valid':
            self.images = orig_images[66000:66500]
            self.labels = orig_labels[66000:66500]
        if mode == 'test':
            self.images = orig_images[66000:]
            self.labels = orig_labels[66000:]
